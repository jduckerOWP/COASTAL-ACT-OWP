...
! $Id: network_data.f90 140618 2022-01-12 13:12:04Z klapwijk $
! $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/releases/140712/src/utils_lgpl/gridgeom/packages/gridgeom/src/network_data.f90 $
...
module network_data
  ...
  character (len=256), allocatable :: q_ids(:), z_ids(:), q_ids_left(:), q_ids_right(:) !< (numl) Edge link boundary condition id
  double precision, allocatable    :: z_lon(:), z_lat(:)
  double precision, allocatable    :: z_lon_left(:), z_lat_left(:), z_lon_right(:), z_lat_right(:)
  double precision, allocatable    :: z_right_weight(:), z_left_weight(:), q_left_weight(:), q_right_weight(:)
