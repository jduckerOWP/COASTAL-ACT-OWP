#define FILENAME "ADCIRC_Hydraulic_Cap"
#define MODNAME "ADCIRC_Hydraulic_Cap"
...
! module ADCIRC Hydraulic Coastal Engine
module ADCIRC_HYC_Cap      
  use ESMF
  use NUOPC
  use NUOPC_Model, &
...
#define METHOD "InitializeP3"
  ...
  subroutine InitializeP3(gcomp, importState, exportState, clock, rc)
    ...
    type(ESMF_LocStream)       :: DFlow_LocStream
    ...
    do fIndex = 1, size(NWM_HYCFieldList)
      ...
      if (exportConnected) then
        ...
        field = NWM_HYCFieldCreate(stdName=NWM_HYCFieldList(fIndex)%stdname, &
                                   mesh=NWM_HYCMesh,locstream=DFLOW_locstream,rc=rc)
        if (ESMF_STDERRORCHECK(rc)) return  ! bail out
...
#define METHOD "ModelAdvance"
  ...
  subroutine ModelAdvance(gcomp, rc)
    ...
    !do while (is%wrap%stepTimer(1) >= timeStep .and. jastop.eq.0 ) ! time loop 
      ! run
