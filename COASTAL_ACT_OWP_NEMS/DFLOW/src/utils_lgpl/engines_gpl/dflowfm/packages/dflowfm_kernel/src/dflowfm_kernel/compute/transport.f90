...
! $Id: transport.f90 140618 2022-01-12 13:12:04Z klapwijk $
! $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/releases/140712/src/engines_gpl/dflowfm/packages/dflowfm_kernel/src/dflowfm_kernel/compute/transport.f90 $
...
!                END DEBUG
           ...
           if (jasal > 0) then
              call tridag_dflow(a,b,c,d,e,constituents(isalt,kb:kt),km)
           endif
           if (jatem > 0) then
              call tridag_dflow(ta,tb,tc,td,e,constituents(itemp,kb:kt),km)
           endif
           if (jased > 0) then
              do j = 1,mxgr
                 call tridag_dflow(sa,sb,sc,sd,e,sed(j,kb:kt),km)
              enddo
           endif
