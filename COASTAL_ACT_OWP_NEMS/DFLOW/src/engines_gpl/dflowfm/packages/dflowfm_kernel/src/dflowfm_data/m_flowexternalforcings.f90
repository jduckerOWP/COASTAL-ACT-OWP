...
! $Id: m_flowexternalforcings.f90 140636 2022-01-17 09:43:24Z noort $
! $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/releases/140712/src/engines_gpl/dflowfm/packages/dflowfm_kernel/src/dflowfm_data/m_flowexternalforcings.f90 $
...
! unstruc.f90
 ...
 character(len=256), allocatable   :: idbndz(:)         !< waterlevel id boundary point
 double precision, allocatable     :: wl_lon(:), wl_lon_left(:), wl_lon_right(:)
 double precision, allocatable     :: wl_lat(:), wl_lat_left(:), wl_lat_right(:)
 double precision, allocatable     :: wl_left_weight(:), wl_right_weight(:)
 double precision, allocatable, target :: zbndz(:), zbndz_test(:), zbndz_t0(:), zbndz_t1(:)      !< [m] waterlevel boundary points function  {"location": "edge", "shape": ["nbndz"]}
 ...
 character(len=256), allocatable   :: idbndu(:)         !< velocity   id boundary point
 character(len=256), allocatable   :: idbndu_left(:), idbndu_right(:)
 double precision, allocatable     :: qid_left_weight(:), qid_right_weight(:)
 double precision, allocatable, target :: zbndu(:)      !< [m/s] velocity   boundary points function   {"location": "edge", "shape": ["nbndu"]}
 double precision, allocatable, target :: zbndq(:), zbndq_test(:), zbndq_t0(:), zbndq_t1(:)      !< [m3/s] discharge  boundary points function   {"location": "edge", "shape": ["nbndu"]}
 ...
 double precision, allocatable, target :: zbndt(:), zbndt_t0(:), zbndt_t1(:)     !< tang.velocity boundary points function
 ...
 double precision, allocatable, target :: zbndn(:), zbndn_t0(:), zbndn_t1(:)      !< norm.velocity boundary points function
