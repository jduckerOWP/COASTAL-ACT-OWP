...
!
  ...
  real(ESMF_KIND_R8), dimension(:), allocatable   :: NWM_CommonID
  real(ESMF_KIND_R8), dimension(:), allocatable   :: NWM_lon
  real(ESMF_KIND_R8), dimension(:), allocatable   :: NWM_lat
  real(ESMF_KIND_R8), dimension(:), allocatable   :: HUCS_ID
  real(ESMF_KIND_R8), dimension(:), allocatable   :: HUCS_unique_ID
  ...
  public :: NWM_CommonID, NWM_lon, NWM_lat, HUCS_ID, HUCS_unique_ID, NWM_HUCS_agg_streamflow
...
#define METHOD "locstream_velx"
  ...
  function locstream_velx(vm, rc)
    ...
    if(vbc_size > 0) then
       ...
       allocate(lat(vbc_size))
       allocate(lon(vbc_size))
       allocate(mask(vbc_size))
       numlocations = vbc_size
       do i = 1, numlocations
          arbSeqIndexList(i) = i
          lat(i) = DFLOW_VBC_lat(i)
          lon(i) = DFLOW_VBC_lon(i)
    ...
    SELECT CASE (trim(stdName))
      ...
      CASE ('surface_northward_sea_water_velocity_dflow')
          ...
          if(size(DFLOW_VBC_lat) > 0) then
              allocate(farrayPtr(size(DFLOW_VBC_lat)))
          else
              allocate(farrayPtr(1))
          endif
...
#define METHOD "NWM_Read_HUCS_agg"
  ...
  DO row = 1, nlines
     READ(rank,*) NWM_CommonID(row), NWM_lat(row), NWM_lon(row), HUCS_ID(row)
  END DO
  ...
  write(*,*) "NWM Common ID int"
  WRITE(*,*) NWM_CommonID(5:10)
...
#define METHOD "DFLOW_VBC_derivation"
  ...
  integer :: id_temp, i, j, unique, unique_index, vel_loop
