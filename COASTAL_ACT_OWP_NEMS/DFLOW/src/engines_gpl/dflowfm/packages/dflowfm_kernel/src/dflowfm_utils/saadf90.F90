...
#include "blasfm.h"
    ...
    END MODULE GAMMAS
     ...
     PUBLIC
      ...
      PUBLIC :: gen57pt,cg_dflow,bcg_dflow,dbcg_dflow,bcgstab_dflow,tfqmr_dflow,gmres_dflow,fgmres_dflow,dqgmres_dflow, cgnr_dflow, fom_dflow, runrc_dflow, ilut_dflow
!
     !end module m_saad
...
!-------------- compute initial residual vector --------------
       call amux_dflow(n, sol, vv, aa, ja, ia)
       do 21 j=1,n
          vv(j,1) = rhs(j) - vv(j,1)
 21    continue
!-------------------------------------------------------------
 20    ro = dnrm2_dflow(n, vv, 1)
...
!     ** initialize 1-st term  of rhs of hessenberg system..
 ...
 4     i=i+1
       ...
       call amux_dflow(n, rhs, vv(1,i1), aa, ja, ia)
!-----------------------------------------
!     modified gram - schmidt...
!-----------------------------------------
       do 55 j=1, i
          t = ddot_dflow(n, vv(1,j),1,vv(1,i1),1)
          hh(j,i) = t
          call daxpy_dflow(n, -t, vv(1,j), 1, vv(1,i1), 1)
 55    continue
       t = dnrm2_dflow(n, vv(1,i1), 1)
       hh(i1,i) = t
...
!
 ...
 24    continue
       do 25  j=1,i1
          t = rs(j)
          if (j .eq. 1)  t = t-1.0d0
          call daxpy_dflow(n, t, vv(1,j), 1,  vv, 1)
 25    continue
...
!
 ...
 10   if ( jabcgstab.eq.1 ) then
         ...
         call CG_dflow(n,rhs,sol,ipar,fpar,wk)
      end if
!
...
!
      if (ipar(1).eq.1) then
         call amux_dflow(n, wk(ipar(8)), wk(ipar(9)), a, ja, ia)
         goto 10
...
!
      ...
      call amux_dflow(n,sol,wk,a,ja,ia)
      do i = 1, n
         wk(n+i) = sol(i) -1.0D0
         wk(i) = wk(i) - rhs(i)
      enddo
...
!-----------------------------------------------------------------------
      ...
      EXTERNAL_DDOT
      distdot = ddot_dflow(n,x,ix,y,iy)
      return
      end
!-----end-of-distdot
!-----------------------------------------------------------------------
...
!                 Residual method                                      c
!----------------------------------------------------------------------c
!     Th_dflowey all have the following calling sequence:
...
!     convergence tests.
!-----------------------------------------------------------------------
      subroutine cg_dflow(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
...
!
      fpar(7) = ddot_dflow(n,w,1,w,1)
      fpar(11) = fpar(11) + 2 * n
      fpar(3) = sqrt(fpar(7))
      fpar(5) = fpar(3)
      if (abs(ipar(3)).eq.2) then
         fpar(4) = fpar(1) * sqrt(ddot_dflow(n,rhs,1,rhs,1)) + fpar(2)
         fpar(11) = fpar(11) + 2 * n
...
!
 60   ipar(7) = ipar(7) + 1
      alpha = ddot_dflow(n,w(1,2),1,w(1,3),1)
      fpar(11) = fpar(11) + 2*n
...
!-----end-of-cg
!-----------------------------------------------------------------------
      subroutine cgnr_dflow(n,rhs,sol,ipar,fpar,wk)
      implicit none
      integer n, ipar(16)
...
!
 60   ipar(7) = ipar(7) + 1
      zzm1 = zz
      zz = ddot_dflow(n,wk(1,3),1,wk(1,3),1)
      fpar(11) = fpar(11) + 2 * n
...
!
 90   ipar(7) = ipar(7) + 1
      alpha = ddot_dflow(n,wk(1,3),1,wk(1,3),1)
      fpar(11) = fpar(11) + 2 * n
      if (brkdn_dflow(alpha,ipar)) goto 900
      alpha = zz / alpha
      do i = 1, n
...
!-----end-of-cgnr
!-----------------------------------------------------------------------
      subroutine bcg_dflow(n,rhs,sol,ipar,fpar,w)
      implicit none
      integer n, ipar(16)
...
!
 60   ipar(7) = ipar(7) + 1
      alpha = ddot_dflow(n,w(1,4),1,w(1,5),1)
      fpar(11) = fpar(11) + 2 * n
      if (brkdn_dflow(alpha,ipar)) goto 900
      alpha = fpar(7) / alpha
      do i = 1, n
...
!
 100  ipar(7) = ipar(7) + 1
      ...
      enddo
      fpar(8) = fpar(7)
      fpar(7) = ddot_dflow(n,w,1,w(1,2),1)
      fpar(11) = fpar(11) + 4 * n
...
!
!     step (2)
      alpha = ddot_dflow(n,w(1,1),1,w(1,5),1)  
      fpar(11) = fpar(11) + 2 * n
      if (brkdn_dflow(alpha, ipar)) goto 900
      alpha = fpar(7) / alpha
      fpar(8) = alpha
...
!
!     step (5)
      omega = ddot_dflow(n,w(1,4),1,w(1,4),1)
      if (omega .ne. 0d0) then 
         fpar(11) = fpar(11) + n + n
         if (brkdn_dflow(omega,ipar)) goto 900
         omega = ddot_dflow(n,w(1,4),1,w(1,3),1) / omega
         fpar(11) = fpar(11) + n + n
         if (brkdn_dflow(omega,ipar)) goto 900
      endif
...
!     step (8): computing new p and rho
      rho = fpar(7)
      fpar(7) = ddot_dflow(n,w(1,2),1,w(1,1),1)
      omega = fpar(9)
...
!-----------------------------------------------------------------------
      subroutine tfqmr_dflow(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      double precision ::  rhs(n), sol(n), fpar(16), w(n,*)
!-----------------------------------------------------------------------
...
!
      fpar(5) = sqrt(ddot_dflow(n,w,1,w,1))
      fpar(3) = fpar(5)
      tao = fpar(5)
      fpar(11) = fpar(11) + n + n
      if (abs(ipar(3)).eq.2) then
...
!
 30   sigma = rho
      rho = ddot_dflow(n,w(1,2),1,w(1,3),1)
      fpar(11) = fpar(11) + n + n
...
!
 ...
 60   ipar(7) = ipar(7) + 1
      ...
      enddo
      sigma = ddot_dflow(n,w(1,2),1,w(1,8),1)
      fpar(11) = fpar(11) + 6 * n
...
!     update I
!
      theta = ddot_dflow(n,w(1,3),1,w(1,3),1) / (tao*tao)
      sigma = one / (one + theta)
      ...
      if (ipar(7).eq.1) then
         if (ipar(3).eq.-1) then
            fpar(3) = eta * sqrt(ddot_dflow(n,w(1,9),1,w(1,9),1))
            fpar(4) = fpar(1)*fpar(3) + fpar(2)
...
!     this is an approximation
      ...
      if (ipar(3).eq.999) then
         ...
         fpar(6) = eta * sqrt(ddot_dflow(n,w(1,9),1,w(1,9),1))
...
!-----end-of-tfqmr
!-----------------------------------------------------------------------
      subroutine fom_dflow(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      double precision ::  rhs(n), sol(n), fpar(16), w(*)
!-----------------------------------------------------------------------
...
!
 ...
 10   ipar(7) = ipar(7) + 1
      ...
      fpar(11) = fpar(11) + n
!
 20   alpha = sqrt(ddot_dflow(n,w,1,w,1))
...
!-----end-of-fom--------------------------------------------------------
!-----------------------------------------------------------------------
      subroutine gmres_dflow(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      double precision ::  rhs(n), sol(n), fpar(16), w(*)
!-----------------------------------------------------------------------
...
!-----end-of-gmres
!-----------------------------------------------------------------------
      subroutine dqgmres_dflow(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      double precision ::  rhs(n), sol(n), fpar(16), w(*)
!-----------------------------------------------------------------------
...
!
 10   ipar(7) = ipar(7) + 1
      ...
      fpar(11) = fpar(11) + n
!
 20   alpha = sqrt(ddot_dflow(n, w(iv+1), 1, w(iv+1), 1))
      fpar(11) = fpar(11) + (n + n)
      if (abs(ipar(3)).eq.2) then
         fpar(4) = fpar(1) * sqrt(ddot_dflow(n,rhs,1,rhs,1)) + fpar(2)
         fpar(11) = fpar(11) + 2*n
...
!
      ...
      if (ipar(3).lt.0) then
         ...
         if (ipar(7).eq.2 .and. ipar(3).eq.-1) then
            fpar(3) = alpha*sqrt(ddot_dflow(n, w(ptrw), 1, w(ptrw), 1))
            fpar(4) = fpar(1) * fpar(3) + fpar(2)
            fpar(6) = fpar(3)
         else
            fpar(6) = alpha*sqrt(ddot_dflow(n, w(ptrw), 1, w(ptrw), 1))
         endif
         fpar(11) = fpar(11) + 2 * n
      else
         fpar(6) = fpar(5)
...
!-----end-of-dqgmres
!-----------------------------------------------------------------------
      subroutine fgmres_dflow(n, rhs, sol, ipar, fpar, w)
      implicit none
      integer n, ipar(16)
      double precision ::  rhs(n), sol(n), fpar(16), w(*)
!-----------------------------------------------------------------------
...
!-----end-of-fgmres
!-----------------------------------------------------------------------
      subroutine dbcg_dflow(n,rhs,sol,ipar,fpar,w)
      implicit none
      integer n,ipar(16)
      double precision ::  rhs(n), sol(n), fpar(16), w(n,*)
!-----------------------------------------------------------------------
...
!
 ...
 110  ipar(7) = ipar(7) + 1
      ...
      fpar(11) = fpar(11) + n
!
 120  fpar(3) = sqrt(ddot_dflow(n,w(1,3),1,w(1,3),1))
      fpar(11) = fpar(11) + n + n
      fpar(5) = fpar(3)
      fpar(7) = fpar(3)
      zeta = fpar(3)
...
!
      if (k .ne. i2) then
         ...
         ss = ddot_dflow(n,w(1,1),1,w(1,4+k),1)
         fpar(11) = fpar(11) + 2*n
         ss1= ss
         ju = ju + 1
         u(ju) = ss
...
!
      t = ddot_dflow(n,w(1,2),1,w(1,1),1)
!
      beta   = sqrt(abs(t))
      delta  = t/beta
!
...
!-----------------------------------------------------------------------
 36   if (ipar(3).eq.999) then
      ...
      fpar(5) = res * sqrt(ddot_dflow(n, w(1,i2+2), 1, w(1,i2+2), 1))
      fpar(11) = fpar(11) + 2 * n
      if (ipar(3).lt.0) then
         fpar(6) = x * sqrt(ddot_dflow(n,w,1,w,1))
         fpar(11) = fpar(11) + 2 * n
         if (ipar(7).le.3) then
            fpar(3) = fpar(6)
            if (ipar(3).eq.-1) then
...
!     computes errors
!
      fpar(5) = sqrt(ddot_dflow(n,r,1,r,1))
      fpar(11) = fpar(11) + 2 * n
      if (ipar(3).lt.0) then
!
!     compute the change in the solution vector
!
         fpar(6) = sx * sqrt(ddot_dflow(n,delx,1,delx,1))
         fpar(11) = fpar(11) + 2 * n
...
!
      nrm0 = ddot_dflow(n,vec(1,ind),1,vec(1,ind),1)
      ops = ops + n + n
      thr = nrm0 * reorth
...
!
      if (full) then
         do 40 i = ind+1, m
            fct = ddot_dflow(n,vec(1,ind),1,vec(1,i),1)
            hh(i) = fct
            do 20 k = 1, n
...
!
      nrm1 = sqrt(ddot_dflow(n,vec(1,ind),1,vec(1,ind),1))
      ops = ops + n + n
 75   hh(ind) = nrm1
