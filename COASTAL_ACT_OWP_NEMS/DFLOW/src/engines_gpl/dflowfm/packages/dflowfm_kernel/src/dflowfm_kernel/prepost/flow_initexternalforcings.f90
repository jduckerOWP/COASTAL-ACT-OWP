...
! $Id: flow_initexternalforcings.f90 140618 2022-01-12 13:12:04Z klapwijk $
! $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/releases/140712/src/engines_gpl/dflowfm/packages/dflowfm_kernel/src/dflowfm_kernel/prepost/flow_initexternalforcings.f90 $
 ...
 if (allocated(kbndz))        deallocate(xbndz,ybndz,idbndz,wl_lon,wl_lat,wl_lon_left,wl_lon_right,wl_lat_left,wl_lat_right,wl_left_weight,wl_right_weight,xy2bndz,zbndz,zbndz_t0,zbndz_t1,kbndz,zbndz0)
 ...
 if (nbndz > 0) then                                 ! now you know the elementsets for the waterlevel bnds
    allocate ( xbndz(nbndz), ybndz(nbndz), idbndz(nbndz), wl_lon(nbndz), wl_lat(nbndz),  wl_lon_left(nbndz), wl_lon_right(nbndz), wl_lat_left(nbndz), wl_lat_right(nbndz), wl_left_weight(nbndz), wl_right_weight(nbndz), xy2bndz(2,nbndz), zbndz(nbndz), zbndz_test(nbndz), zbndz_t0(nbndz), zbndz_t1(nbndz), kbndz(n4,nbndz), zbndz0(nbndz), kdz(nbndz) , stat=ierr     )
    call aerr('xbndz(nbndz), ybndz(nbndz), idbndz(nbndz), wl_lon(nbndz), wl_lat(nbndz), wl_lon_left(nbndz), wl_lon_right(nbndz), wl_lat_left(nbndz), wl_lat_right(nbndz), wl_left_weight(nbndz), wl_right_weight(nbndz), xy2bndz(2,nbndz), zbndz(nbndz), zbndz_test(nbndz), zbndz_t0(nbndz), zbndz_t1(nbndz), kbndz(n4,nbndz), zbndz0(nbndz), kdz(nbndz)', ierr, nbndz*10 )
    ...
    do k = 1, nbndz
       ...
       wl_lon(k)    = z_lon(L)
       wl_lat(k)    = z_lat(L)
       wl_lon_left(k) = z_lon_left(L)
       wl_lat_left(k) = z_lat_left(L)
       wl_lon_right(k) = z_lon_right(L)
       wl_lat_right(k) = z_lat_right(L)
       wl_left_weight(k) = z_left_weight(L)
       wl_right_weight(k) = z_right_weight(L)
       zbndz0(k)    = dmiss
       xy2bndz(:,k) = xyen(:,L)
 ...
 if (allocated(kbndu)) deallocate(  xbndu,ybndu,idbndu,xy2bndu,zbndu,kbndu,zbndu0)
 if (allocated(zkbndu)) deallocate( zkbndu)
 if (allocated(zbndq)) deallocate(  zbndq)
 if (allocated(zbndq_test)) deallocate(  zbndq_test)
 if (allocated(zbndq_t0)) deallocate(  zbndq_t0)
 if (allocated(zbndq_t1)) deallocate(  zbndq_t1)
 if (allocated(sigmabndu)) deallocate(sigmabndu)
 if (allocated(zminmaxu)) deallocate(zminmaxu)
 if (nbndu > 0) then                                 ! similar for u bnd's
    allocate ( xbndu(nbndu), ybndu(nbndu), idbndu(nbndu), idbndu_left(nbndu), idbndu_right(nbndu), qid_left_weight(nbndu), qid_right_weight(nbndu), xy2bndu(2,nbndu), kbndu(n4,nbndu), kdu(nbndu) , stat=ierr)
    call aerr('xbndu(nbndu), ybndu(nbndu), idbndu(nbndu), idbndu_left(nbndu), idbndu_right(nbndu), qid_left_weight(nbndu), qid_right_weight(nbndu), xy2bndu(2,nbndu), kbndu(n4,nbndu), kdu(nbndu)', ierr, nbndu*(n4+5) )
    ...
    allocate ( zbndq_test    (nbndu) , stat = ierr       )
    call aerr('zbndq_test    (nbndu)', ierr , nbndu      )

    allocate ( zbndq_t0    (nbndu) , stat = ierr       )
    call aerr('zbndq_t0    (nbndu)', ierr , nbndu      )


    allocate ( zbndq_t1    (nbndu) , stat = ierr       )
    call aerr('zbndq_t1    (nbndu)', ierr , nbndu      )
    ...
    do k = 1, nbndu
       ...
       ybndu(k)   = ye(L) ! yz(kb)
       idbndu(k)  = q_ids(L)
       idbndu_left(k) = q_ids_left(L)
       idbndu_right(k) = q_ids_right(L)
       qid_left_weight(k) = q_left_weight(L)
       qid_right_weight(k) = q_right_weight(L)
       xy2bndu(:,k) = xyen(:,L)
...
!   allocate
 ...
 if (allocated   (kbndt) ) deallocate(xbndt, ybndt, xy2bndt, zbndt, zbndt_t0, zbndt_t1, kbndt)
 if (nbndt > 0) then                                 ! Tangential velocity boundaries as u bnds
    numnos = 0
    allocate ( xbndt(nbndt), ybndt(nbndt), xy2bndt(2,nbndt), zbndt(nbndt), zbndt_t0(nbndt), zbndt_t1(nbndt), kbndt(4,nbndt), kdt(nbndt) , stat=ierr     )
    call aerr('xbndt(nbndt), ybndt(nbndt), xy2bndt(2,nbndt), zbndt(nbndt), zbndt_t0(nbndt), zbndt_t1(nbndt), kbndt(4,nbndt), kdt(nbndt)', ierr, nbndt*10 )
    kbndt = 0 ; kdt= 1
    do k = 1, nbndt
       L          = ket(k)
       Lf         = lne2ln(L)
 ...
 if (allocated   (kbndn) ) deallocate(  xbndn,ybndn,xy2bndn,zbndn,zbndn_t0,zbndn_t1,kbndn)
 if (nbndn > 0) then                                 ! Normal velocity boundaries as z bnds
    numnos = 0
    allocate ( xbndn(nbndn), ybndn(nbndn), xy2bndn(2,nbndn), zbndn(nbndn), zbndn_t0(nbndn), zbndn_t1(nbndn), kbndn(4,nbndn), kdn(nbndn) , stat=ierr     )
    call aerr('xbndn(nbndn), ybndn(nbndn), xy2bndn(2,nbndn), zbndn(nbndn), zbndn_t0(nbndn), zbndn_t1(nbndn), kbndn(4,nbndn), kdn(nbndn)', ierr, nbndn*10 )
    kbndn = 0 ; kdn= 1
    do k = 1, nbndn
       L          = ken(k)
       Lf         = lne2ln(L)
...
!          success = ec_addtimespacerelation(qid, xcdam, ycdam, kdd, filename, filetype, method, operand, xy2cdam)
 ...
 if (num_lat_ini_blocks == 0 .and. numlatsg > 0) then ! Allow laterals from old ext, even when new structures file is present (but only when *no* [Lateral]s were in new extforce file).
    ...
    call realloc(qplat_test, numlatsg, keepExisting = .false., fill = 0d0)
