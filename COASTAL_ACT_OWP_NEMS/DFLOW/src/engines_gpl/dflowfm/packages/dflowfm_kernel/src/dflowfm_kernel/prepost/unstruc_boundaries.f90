...
! $Id: unstruc_boundaries.f90 140618 2022-01-12 13:12:04Z klapwijk $
! $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/releases/140712/src/engines_gpl/dflowfm/packages/dflowfm_kernel/src/dflowfm_kernel/prepost/unstruc_boundaries.f90 $
...
! endif
 ...
 if (allocated(z_lon)) deallocate(z_lon)
 
 if (allocated(z_lat)) deallocate(z_lat)

 if (allocated(z_lon_left)) deallocate(z_lon_left)

 if (allocated(z_lat_left)) deallocate(z_lat_left)

 if (allocated(z_lon_right)) deallocate(z_lon_right)

 if (allocated(z_lat_right)) deallocate(z_lat_right)

 if (allocated(z_left_weight)) deallocate(z_left_weight)

 if (allocated(z_right_weight)) deallocate(z_right_weight)

 if (allocated(q_left_weight)) deallocate(q_left_weight)

 if (allocated(q_right_weight)) deallocate(q_right_weight)
...
! count number of 2D links and 1D endpoints
 ...
 allocate( q_ids(nx), stat=ierr) ; q_ids = ''
 call aerr("q ids (nx)", ierr, nx)
 ...
 allocate( z_ids(nx), stat=ierr) ; z_ids = ''
 call aerr("z ids (nx)", ierr, nx)

 allocate ( z_lon(nx), stat=ierr ) ; z_lon = 0
 call aerr('z_lon (nx)',     ierr, nx)

 allocate ( z_lat(nx), stat=ierr ) ; z_lat = 0
 call aerr('z_lat (nx)',     ierr, nx)

 allocate ( z_lon_left(nx), stat=ierr ) ; z_lon_left = 0
 call aerr('z_lon_left (nx)',     ierr, nx)

 allocate ( z_lat_left(nx), stat=ierr ) ; z_lat_left = 0
 call aerr('z_lat_left (nx)',     ierr, nx)

 allocate ( z_lon_right(nx), stat=ierr ) ; z_lon_right = 0
 call aerr('z_lon_right (nx)',     ierr, nx)

 allocate ( z_lat_right(nx), stat=ierr ) ; z_lat_right = 0
 call aerr('z_lat_right (nx)',     ierr, nx)

 allocate ( z_left_weight(nx), stat=ierr ) ; z_left_weight = 0
 call aerr('z_left_weight (nx)',     ierr, nx)

 allocate ( z_right_weight(nx), stat=ierr ) ; z_right_weight = 0
 call aerr('z_right_weight (nx)',     ierr, nx)

 allocate ( q_left_weight(nx), stat=ierr ) ; q_left_weight = 0
 call aerr('q_left_weight (nx)',     ierr, nx)

 allocate ( q_right_weight(nx), stat=ierr ) ; q_right_weight = 0
 call aerr('q_right_weight (nx)',     ierr, nx)
 ...
 do i=1,num_items_in_file
    ...
    if (strcmpi(groupname, 'Boundary')) then
       ...
       if (property_ok)  then
          ...
          call mess(LEVEL_WARN, 'One of the boundary blocks in file '''//trim(filename)//'''(QUANTITY = '''//trim(quantity)//''') is missing a property ''forcingfile''. Make sure forcing data is connected and coming from NUOPC cap.')
       end if
...
logical function initboundaryblocksforcings(filename)
 ...
 if (maxlatsg > 0) then
    ...
    call realloc(qplat_test, maxlatsg, keepExisting = .false., fill = 0d0)
 ...
 do i=1,num_items_in_file
    ...
    case ('boundary')
       ...
       if (retVal) then
          ...
          write(msgbuf, '(5a)') 'Possibly incomplete block in file ''', trim(filename), ''': [', trim(groupname), ']. Field ''forcingFile'' is missing. Make sure forcing data is connected and coming from NUOPC cap.'
          call warn_flush()
          !cycle
          forcingfile = ''
       end if
    ...
    case ('lateral')
       ...
       if (len_trim(rec) > 0) then
          ...
          write(msgbuf, '(a,a,a)') 'Required field ''discharge'' missing in lateral ''', trim(locid), '''. Make sure the forcing data is connected and coming from NUOPC cap.'
          call warn_flush()
          !cycle
       end if
