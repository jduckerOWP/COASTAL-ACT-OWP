...
module adc_cap
  ...
  USE GLOBAL, ONLY: QNIN2                ! Import wave 2D discharge to eventually ingest from National Water model
 ...
 ! USE GLOBAL,  ONLY: WTIMINC             ! wind time interval  may be set in ATM.cap or ........  <<:TODO:
  ...
  use adc_mod, only: locstream_waterlevel, locstream_velx, locstream_vely
...
!
  ...
  subroutine ADCIRC_FieldsSetup
    ...
    call NUOPC_FieldDictionaryAddEntry("sea_surface_height_above_sea_level_dflow", "zeta_dflow", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out

    call NUOPC_FieldDictionaryAddEntry("surface_eastward_sea_water_velocity_dflow", "velx_dflow", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out

    call NUOPC_FieldDictionaryAddEntry("surface_northward_sea_water_velocity_dflow", "vely_dflow", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
    ...
    call fld_list_add(num=fldsFrAdc_num, fldlist=fldsFrAdc, stdname="sea_surface_height_above_sea_level_dflow",  shortname= "zeta_dflow" )
    call fld_list_add(num=fldsFrAdc_num, fldlist=fldsFrAdc, stdname="surface_eastward_sea_water_velocity", shortname= "velx" )
    call fld_list_add(num=fldsFrAdc_num, fldlist=fldsFrAdc, stdname="surface_eastward_sea_water_velocity_dflow", shortname= "velx_dflow" )
    call fld_list_add(num=fldsFrAdc_num, fldlist=fldsFrAdc, stdname="surface_northward_sea_water_velocity",shortname= "vely" )
    call fld_list_add(num=fldsFrAdc_num, fldlist=fldsFrAdc, stdname="surface_northward_sea_water_velocity_dflow",shortname= "vely_dflow" )
...
!
  subroutine InitializeP2(model, importState, exportState, clock, rc)
    ...
    real(ESMF_KIND_R8), pointer:: dataPtr_zeta_dflow(:)
    real(ESMF_KIND_R8), pointer:: dataPtr_velx(:)
    real(ESMF_KIND_R8), pointer:: dataPtr_velx_dflow(:)
    real(ESMF_KIND_R8), pointer:: dataPtr_vely(:)
    real(ESMF_KIND_R8), pointer:: dataPtr_vely_dflow(:)

   
    type(ESMF_LocStream)       :: adcirc_waterlevel, adcirc_velx, adcirc_vely
    character(len=128)         :: fldname, timeStr
    integer                    :: i1,num
    logical :: surge_forcing = .false. ; 
    logical :: dflow_forcing = .false. ;
    ! DW
    ...
    ! Create Locstream ESMF field for ADCIRC waterlevels to send to D-Flow
    ! boundaries
    adcirc_waterlevel = locstream_waterlevel(localPet,rc,mdata)

    adcirc_velx = locstream_velx(localPet,rc,mdata)
    adcirc_vely = locstream_vely(localPet,rc,mdata)
    ...
    !print *,"ADC ..4.............................................. >> "
    call ADCIRC_RealizeFields(importState, meshIn , mdata, adcirc_waterlevel, adcirc_velx, adcirc_vely, fldsToAdc_num, fldsToAdc, "Adcirc import", rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    !
    call ADCIRC_RealizeFields(exportState, meshOut, mdata, adcirc_waterlevel, adcirc_velx, adcirc_vely, fldsFrAdc_num, fldsFrAdc, "Adcirc export", rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
...
!        rc=rc,dump=.true.,timeStr=timeStr)
    ...
    ! Initialized ADCIRC export fields (zeta) for D-Flow model component
    dflow_forcing= .true.
    do num = 1, fldsFrAdc_num
       if ( fldsFrAdc(num)%shortname == 'zeta_dflow' ) dflow_forcing = dflow_forcing .and. fldsFrAdc(num)%connected
       if ( fldsFrAdc(num)%shortname == 'velx_dflow' ) dflow_forcing = dflow_forcing .and. fldsFrAdc(num)%connected
       if ( fldsFrAdc(num)%shortname == 'vely_dflow' ) dflow_forcing = dflow_forcing .and. fldsFrAdc(num)%connected
    end do

    if (dflow_forcing) then
      !-----------------------------------------
      !   EXPORT
      !-----------------------------------------
      !pack and send exported fields: zeta, velx, vely
      ...
      ! >>>>> PACK and send ZETA
      call State_getFldPtr_(ST=exportState,fldname='zeta_dflow',fldptr=dataPtr_zeta_dflow,rc=rc,dump=.false.,timeStr=timeStr)
      ...
      !fill only owned nodes for tmp vector
      !do i1 = 1, mdataOut%NumOwnedNd, 1
      !  dataPtr_zeta_dflow(i1) = ETA2(mdataOut%owned_to_present_nodes(i1)) ;
      !end do


      do i1 = 1, size(sfea), 1
        dataPtr_zeta_dflow(i1) = ETA2(i1) ;
      end do

      ! >>>>> PACK and send VELX
      call State_getFldPtr_(ST=exportState,fldname='velx_dflow',fldptr=dataPtr_velx_dflow,rc=rc,dump=.false.,timeStr=timeStr)
      ...
      !fill only owned nodes for tmp vector
      !do i1 = 1, mdataOut%NumOwnedNd, 1
      !   dataPtr_velx_dflow(i1) = UU2(mdataOut%owned_to_present_nodes(i1)) ;
      !end do

      do i1 = 1, size(sfea), 1
        dataPtr_velx_dflow(i1) = UU2(i1) ;
      end do

      ! >>>>> PACK and send VELY
      call State_getFldPtr_(ST=exportState,fldname='vely_dflow',fldptr=dataPtr_vely_dflow,rc=rc,dump=.false.,timeStr=timeStr)
      ...
      !fill only owned nodes for tmp vector
      !do i1 = 1, mdataOut%NumOwnedNd, 1
      !   dataPtr_vely_dflow(i1) = VV2(mdataOut%owned_to_present_nodes(i1)) ;
      !end do

      do i1 = 1, size(sfea), 1
        dataPtr_vely_dflow(i1) = VV2(i1) ;
      end do

    else
      write(info,*) subname,' --- no dflow coastal modeling with water levels coupled with ADC'
      call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=dbrc)
    endif
 ...
 !> Adds a set of fields to an ESMF_State object.  Each field is wrapped
  ...
  !! @param rc return code
  subroutine ADCIRC_RealizeFields(state, mesh, mdata, waterlevel_dflow, velx_dflow, vely_dflow , nfields, field_defs, tag, rc)

    type(ESMF_State), intent(inout)             :: state
    type(ESMF_Mesh), intent(in)                 :: mesh
    type(meshdata)                              :: mdata
    type(ESMF_LocStream), intent(in)            :: waterlevel_dflow
    type(ESMF_LocStream), intent(in)            :: velx_dflow
    type(ESMF_LocStream), intent(in)            :: vely_dflow
    integer, intent(in)                         :: nfields
    type(fld_list_type), intent(inout)          :: field_defs(:)
    character(len=*), intent(in)                :: tag
    integer, intent(inout)                      :: rc

    real(ESMF_KIND_R8), dimension(:), pointer   :: waterlevel_ptr => null()
    real(ESMF_KIND_R8), dimension(:), pointer   :: velx_ptr => null()
    real(ESMF_KIND_R8), dimension(:), pointer   :: vely_ptr => null()
    type(ESMF_Field)                            :: field
    integer                                     :: i
    ...
    do i = 1, nfields
        ! If ADCIRC realized field is waterlevels, then we must initalize the
        ! field as a locstream variable
        if(field_defs(i)%shortname .eq. 'zeta_dflow') then
            allocate(waterlevel_ptr(size(sfea)))
            field = ESMF_FieldCreate(locstream=waterlevel_dflow, farrayPtr=waterlevel_ptr, &
              datacopyflag=ESMF_DATACOPY_REFERENCE, name=field_defs(i)%shortname, rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
              line=__LINE__, &
              file=__FILE__)) &
              return  ! bail out

        elseif(field_defs(i)%shortname .eq. 'velx_dflow') then
            allocate(velx_ptr(size(sfea)))
            field = ESMF_FieldCreate(locstream=velx_dflow, farrayPtr=velx_ptr, &
              datacopyflag=ESMF_DATACOPY_REFERENCE, name=field_defs(i)%shortname, rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
              line=__LINE__, &
              file=__FILE__)) &
        ...
        elseif(field_defs(i)%shortname .eq. 'vely_dflow') then
            allocate(vely_ptr(size(sfea)))
            field = ESMF_FieldCreate(locstream=vely_dflow, farrayPtr=vely_ptr, &
              datacopyflag=ESMF_DATACOPY_REFERENCE, name=field_defs(i)%shortname, rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
              line=__LINE__, &
              file=__FILE__)) &
        ...
        ! Other ADCIRC fields of concern are realized as ESMF fields
        ! originating from its mesh
...
!  end subroutine
  ...
  !! model advance then the model fail, but COASTAL Act models initalize in 
  !! different phases, so we must include this to tell NEMS to bypass the time
  !! check and allow the models to communicate with each other. 
  !-----------------------------------------------------------------------------
#undef METHOD
...
subroutine CheckImport(model,rc)
    ...
    if (.not.allCurrTime) then
      call ESMF_LogWrite(trim(cname)//": ADCIRC NUOPC TIME CHECK MISTMATCH:"// &
        "Import Fields not at correct time, but bypassing error because models initalization phase likely differ. Please double check to make sure this is the case.", &
        ESMF_LOGMSG_WARNING)
    endif
...
!
 ...
 !           MAXVAL(WVNY1), MAXVAL(WVNY2) ; 
   ...
   do num = 1,fldsFrAdc_num
    ...
    ! Initialized ADCIRC export fields (zeta, velx, vely) for D-Flow model component
    dflow_forcing= .true.
    do num = 1, fldsFrAdc_num
       if ( fldsFrAdc(num)%shortname == 'zeta_dflow' ) dflow_forcing = dflow_forcing .and. fldsFrAdc(num)%connected
       if ( fldsFrAdc(num)%shortname == 'velx_dflow' ) dflow_forcing = dflow_forcing .and. fldsFrAdc(num)%connected
    ...
    if (dflow_forcing) then
      ...
      !WRITE(*,*) "ADCIRC zeta vals"
      !WRITE(*,*) maxval(dataPtr_zeta_dflow)
      !WRITE(*,*) minval(dataPtr_zeta_dflow)
      ...
      write(info,*) subname,' --- no dflow coastal modeling with water levels and velocity components coupled with ADC'
      call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=dbrc)
    endif
