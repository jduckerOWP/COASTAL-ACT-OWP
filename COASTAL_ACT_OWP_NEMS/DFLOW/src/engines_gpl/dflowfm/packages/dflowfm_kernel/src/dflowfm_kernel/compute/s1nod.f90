...
! $Id: s1nod.f90 140618 2022-01-12 13:12:04Z klapwijk $
! $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/releases/140712/src/engines_gpl/dflowfm/packages/dflowfm_kernel/src/dflowfm_kernel/compute/s1nod.f90 $
...
!      Dirichlet boundary condition
   ...
   if(bbr(kb) .eq. 0.0) then
   write(*,*) "bbr is zero. What is zbndz?"
   write(*,*) zbndz(n)
   elseif(zbndz(n) .eq. 0.0) then
   write(*,*) "zbndz is zero. This should not happen"
   write(*,*) bbr(n)
   elseif(ddr(kb) .eq. 0.0) then
   write(*,*) "ddr is zero. What is zbndn, ccr, dx?"
   write(*,*) zbndz(n)
   write(*,*) ccr(Lv2(L))
   write(*,*) dx(L)
   endif
