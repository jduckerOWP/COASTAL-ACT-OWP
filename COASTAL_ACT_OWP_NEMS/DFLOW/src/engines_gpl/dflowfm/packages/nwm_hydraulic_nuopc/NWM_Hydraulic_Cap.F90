...
#define METHOD "ModelAdvance"
  ...
  subroutine ModelAdvance(gcomp, rc)
    ...
    type(ESMF_VM)               :: vm
    type(ESMF_Field)                       :: field, field2, field3
    type(ESMF_LocStream)                   :: locstream, locstream_NWM, locstream2
    integer                                :: itemCount
    ...
    real(ESMF_KIND_R8) :: esmfstep, esmft, esmfnext, latq_agg

    ! imported variables
    real(ESMF_KIND_R8), pointer:: link(:), dataPtr_streamflow(:), lat(:), lon(:), lat2(:), NWM_lat(:), NWM_lon(:), NWM_link(:), waterlevel_ptr(:), velx_ptr(:), vely_ptr(:), lateral_ptr(:), lateral_id(:)
    ...
    real(ESMF_KIND_R8), dimension(:), allocatable :: Distance_left, UTANx_left, UTANy_left, UNORx_left, UNORy_left, TanV_left, NorV_left
    real(ESMF_KIND_R8), dimension(:), allocatable :: Distance_right, UTANx_right, UTANy_right, UNORx_right, UNORy_right, TanV_right, NorV_right


    integer :: vel_loop, dir_loop, HUC_count, NWM_count, velx_loop, vely_loop, test_loop, test_loop2, vel_index, i, j
    logical :: vel_connection

    ! EC-Module time interpolation variables
    real(ESMF_KIND_R8) :: a0, a1, t0, t1, diff, time_dt, dt_user_count

    ! DFlow run execution variables
    integer :: jastop=0, iresult = DFM_NOERR, dummykey
      ...
      if ( itemTypeList(iIndex) == ESMF_STATEITEM_FIELD) then
        ...
        if(itemNameList(iIndex)=='flow_rate') then
                ...
                if(nbndu > 0) then
                    ...
                    !test_loop = 1
                    !do test_loop2 = 1, nbndu
                    !zbndq_test(test_loop2) = (dataPtr_streamflow(test_loop)*qid_left_weight(test_loop2)) + (dataPtr_streamflow(test_loop+1)*qid_right_weight(test_loop2))
                    !test_loop = test_loop + 2
                    !enddo

                    if(time_user .eq. tstart_user) then
                        DFLOW_zbndq_t0(:) = 0.0
                        DFLOW_zbndq_t1(:) = dataPtr_streamflow
                        !zbndq_t0(1:nbndu) = 0.0
                        !zbndq_t1(1:nbndu) = dataPtr_streamflow(1:nbndu)
                        !zbndq_t1(1:nbndu) = zbndq_test(1:nbndu)
                    else
                        DFLOW_zbndq_t0(:) = DFLOW_zbndq_t1(:)
                        DFLOW_zbndq_t1(:) = dataPtr_streamflow
                        !zbndq_t0(1:nbndu) = zbndq_t1(1:nbndu)
                        !zbndq_t1(1:nbndu) = dataPtr_streamflow(1:nbndu)
                        !zbndq_t1(1:nbndu) = zbndq_test(1:nbndu)
                    endif
                    !test_loop = 1
                    !do test_loop2 = 1, nbndu
                    !zbndq(test_loop2) = (dataPtr_streamflow(test_loop)*qid_left_weight(test_loop2)) + (dataPtr_streamflow(test_loop+1)*qid_right_weight(test_loop2))
                    !test_loop = test_loop + 2
                    !enddo

                    !zbndq(1:nbndu) = 0.7
                    !call ESMF_StateGet(importState, itemName='flow_rate', field=field3,rc=rc)
                    !if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
                    !call ESMF_FieldGet(field3, locstream=locstream2, rc=rc)
                    !if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,line=__LINE__, file=__FILE__)) return

                    !call ESMF_LocStreamGetKey(locstream2, "ESMF:Lat", farray=lat2,rc=rc)
                    !if (ESMF_STDERRORCHECK(rc)) return
                    ...
                    !write(*,*) lat2(1:nbndu)
                    !write(*,*) "NEMS flowrate"
                    !write(*,*) dataPtr_streamflow(1:4)
                    !write(*,*) "EC Module flowrate"
                    !write(*,*) zbndq(1:2)
                    !write(*,*) "Discharge min and max"
                    !write(*,*) minval(dataPtr_streamflow(1:nbndu))
                    !write(*,*) maxval(dataPtr_streamflow(1:nbndu))
                    !WRITE(*,*) "Discharge infinite check"
                    !WRITE(*,*) HUGE(dataPtr_streamflow(1:nbndu))
                    !WRITE(*,*) maxval(dataPtr_streamflow(1:nbndu))
                    !DO test_loop = 1, nbndu
                    !if(dataPtr_streamflow(test_loop)*0.0 .ne. 0.0) then
                    !WRITE(*,*) "Discharge data has NAN Value"
                    !ENDIF
                    !ENDDO
        ...
        elseif(itemNameList(iIndex)=='lateral_flow_rate') then
                ...
                !NWM_HUCS_agg_streamflow(:) = 0.0
                 ...
                 do unique_index = 1, size(HUCS_unique_ID)
                        ...
                        if(HUCS_ID(csv_index) .eq. HUCS_unique_ID(unique_index)) then
                           do NWM_loop = 1, size(lateral_id)
                              if(lateral_id(NWM_loop) .eq. NWM_commonID(csv_index)) then
                                 !NWM_index = NWM_loop
                                 latq_agg = latq_agg + lateral_ptr(NWM_loop)
                                 HUC_count = HUC_count + 1
                                 if(HUC_count > 70 .and. HUCS_unique_ID(unique_index) .eq. 30502010605.0) then
                                 write(*,*) "latq agg"
                                 write(*,*) HUC_count
                                 write(*,*) latq_agg
                                 endif
                           ...
                           !if(HUCS_unique_ID(unique_index) .eq. 30502010605.0)  then
                    ...
                    NWM_HUCS_agg_streamflow(unique_index) = latq_agg
                    ...
                    do latq_index = 1, numlatsg
                       ...
                       inner : do unique_index = 1, size(HUCS_unique_ID)
                          if(latq_ids(latq_index) .eq. HUCS_unique_ID(unique_index)) then
                             if(time_user .eq. tstart_user) then 
                                 qplat_t0(latq_index) = 0.0
                                 qplat_t1(latq_index) = NWM_HUCS_agg_streamflow(unique_index)
                             else
                                 qplat_t0(latq_index) = qplat_t1(latq_index)
                                 qplat_t1(latq_index) = NWM_HUCS_agg_streamflow(unique_index)
                             endif
                             ...
                             !write(*,*) "NEMS data"
                             !write(*,*) NWM_HUCS_agg_streamflow(unique_index)
                             !write(*,*) "EC Module data"
                             !write(*,*) qplat(latq_index)
                             !endif
                             if(HUCS_unique_ID(unique_index) .eq. 30502010605) then
                             write(*,*) "NEMS 30502010605 data"
                             write(*,*) NWM_HUCS_agg_streamflow(unique_index)
                             write(*,*) "EC Module data"
                             write(*,*) qplat(latq_index)
                             elseif(HUCS_unique_ID(unique_index) .eq. 30402080303) then
                             write(*,*) "NEMS 30402080303 data"
                             write(*,*) NWM_HUCS_agg_streamflow(unique_index)
                             write(*,*) "EC Module data"
                             write(*,*) qplat(latq_index)
                             elseif(HUCS_unique_ID(unique_index) .eq. 30402080402) then
                             write(*,*) "NEMS 30402080402 data"
                             write(*,*) NWM_HUCS_agg_streamflow(unique_index)
                             write(*,*) "EC Module data"
                             write(*,*) qplat(latq_index)
                             !elseif(HUCS_unique_ID(unique_index) .eq. 30202020506) then
                             !write(*,*) "NEMS 30202020506 data"
                             !write(*,*) NWM_HUCS_agg_streamflow(unique_index)
                             !write(*,*) "EC Module data"
                             !write(*,*) qplat(latq_index)
                             !elseif(HUCS_unique_ID(unique_index) .eq. 30201030404) then
                             !write(*,*) "NEMS 30201030404 data"
                             !write(*,*) NWM_HUCS_agg_streamflow(unique_index)
                             !write(*,*) "EC Module data"
                             !write(*,*) qplat(latq_index)
                             !elseif(HUCS_unique_ID(unique_index) .eq. 30502060308) then
                             !write(*,*) "NEMS 30502060308 data"
                             !write(*,*) NWM_HUCS_agg_streamflow(unique_index)
                             !write(*,*) "EC Module data"
                             !write(*,*) qplat(latq_index)
                             endif
                ...
                !    write(*,*) qplat(1:2)
                    ...
                    !WRITE(*,*) "Lateral discharge infinite check"
                    !WRITE(*,*) HUGE(qplat(1:numlatsg))
                    !WRITE(*,*) maxval(qplat(1:numlatsg))
                    !qplat(1:numlatsg) = 1.0
                    !DO test_loop = 1, numlatsg
                    !if(qplat(test_loop)*0.0 .ne. 0.0) then
                    !WRITE(*,*) "Lateral data has NAN Value"
                    !ENDIF
                    !ENDDO
                !endif
        ...
        elseif(itemNameList(iIndex)=='sea_surface_height_above_sea_level_dflow') then
               ...
               !     write(*,*) zbndz(1:nbndz)
                    !if(time_user == 345600.0) then
                    !    zbndz(1:nbndz) = 0.0
                    !    write(*,*) "set water levels to zero for first timestep"
                    !    write(*,*) time_user
                    !else

                    !test_loop = 1
                    !do test_loop2 = 1, nbndz
                    !zbndz_test(test_loop2) = (waterlevel_ptr(test_loop)*wl_left_weight(test_loop2)) + (waterlevel_ptr(test_loop+1)*wl_right_weight(test_loop2))
                    !test_loop = test_loop + 2
                    !enddo


                    !if(time_user .eq. tstart_user) then
                    !    zbndz_t0(1:nbndz) = 0.0
                    !    !zbndz_t1(1:nbndz) = waterlevel_ptr(1:nbndz)
                    !    zbndz_t1(1:nbndz) = zbndz_test(1:nbndz)
                    !    write(*,*) "start time"
                    !else
                    !    zbndz_t0(1:nbndz) = zbndz_t1(1:nbndz)
                    !    !zbndz_t1(1:nbndz) = waterlevel_ptr(1:nbndz)
                    !    zbndz_t1(1:nbndz) = zbndz_test(1:nbndz)
                    !    write(*,*) "next!"
                    !endif
                    ...
                    if(time_user .eq. tstart_user) then
                        DFLOW_zbndz_t0(:) = 0.0
                        DFLOW_zbndz_t1(:) = waterlevel_ptr(:)
                    else
                        DFLOW_zbndz_t0(:) = DFLOW_zbndz_t1(:)
                        DFLOW_zbndz_t1(:) = waterlevel_ptr(:)
                    endif
                    ...
                    !test_loop = 1
                    !do test_loop2 = 1, nbndz                    
                    !zbndz(test_loop2) = (waterlevel_ptr(test_loop)*wl_left_weight(test_loop2)) + (waterlevel_ptr(test_loop+1)*wl_right_weight(test_loop2))
                    !test_loop = test_loop + 2
                    !enddo
                    ...
                    !    write(*,*) "hour 1 onwards, assign NEMS data"
                    !endif
                    !zbndz(1:nbndz) = 0.5
               ...
               !     write(*,*) lon(1:6)
                    ...
                    !WRITE(*,*) "Water level infinite check"
                    !WRITE(*,*) HUGE(waterlevel_ptr(1:nbndz))
                    !WRITE(*,*) maxval(waterlevel_ptr(1:nbndz))
                    !zbndz(1:nbndz) = 0.2
                    !DO test_loop = 1, nbndz
                    !if(waterlevel_ptr(test_loop)*0.0 .ne. 0.0) then
                    !WRITE(*,*) "Water level data has NAN Value"
                    !if(idbndz(test_loop)(1:13) .eq. "CONT_BND_0100") then
                    !write(*,*) "CONT_BND_0100 potential issue"
                    !WRITE(*,*) "NEMS data"
                    !write(*,*) waterlevel_ptr(test_loop)
                    !write(*,*) "EC Module data"
                    !write(*,*) zbndz(test_loop)
                    !write(*,*) "coordinates"
                    !write(*,*) xbndz(test_loop)
                    !write(*,*) ybndz(test_loop)
                    !write(*,*) "bnd coordinates"
                    !write(*,*) wl_lon(test_loop)
                    !write(*,*) wl_lat(test_loop)
                    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0010") then
                    !write(*,*) "CONT_BND_0010 potential issue"
                    !WRITE(*,*) "NEMS data"
                    !write(*,*) waterlevel_ptr(test_loop)
                    !write(*,*) "EC Module data"
                    !write(*,*) zbndz(test_loop)
                    ...
                    !write(*,*) wl_lon(test_loop)
                    !write(*,*) wl_lat(test_loop)
                    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0020") then
                    !write(*,*) "CONT_BND_0020 potential issue"
                    !WRITE(*,*) "NEMS data"
                    !write(*,*) waterlevel_ptr(test_loop)
                    !write(*,*) "EC Module data"
                    !write(*,*) zbndz(test_loop)
                    ...
                    !write(*,*) wl_lon(test_loop)
                    !write(*,*) wl_lat(test_loop)
                    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0040") then
                    !write(*,*) "CONT_BND_0040 potential issue"
                    !WRITE(*,*) "NEMS data"
                    !write(*,*) waterlevel_ptr(test_loop)
                    !write(*,*) "EC Module data"
                    !write(*,*) zbndz(test_loop)
                    ...
                    !write(*,*) wl_lon(test_loop)
                    !write(*,*) wl_lat(test_loop)
                    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0045") then
                    !write(*,*) "CONT_BND_0045 potential issue"
                    !WRITE(*,*) "NEMS data"
                    !write(*,*) waterlevel_ptr(test_loop)
                    !write(*,*) "EC Module data"
                    !write(*,*) zbndz(test_loop)
                    ...
                    !write(*,*) wl_lon(test_loop)
                    !write(*,*) wl_lat(test_loop)
                    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0075") then
                    !write(*,*) "CONT_BND_0075 potential issue"
                    !WRITE(*,*) "NEMS data"
                    !write(*,*) waterlevel_ptr(test_loop)
                    !write(*,*) "EC Module data"
                    !write(*,*) zbndz(test_loop)
                    ...
                    !write(*,*) wl_lon(test_loop)
                    !write(*,*) wl_lat(test_loop)
                    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0165") then
                    !write(*,*) "CONT_BND_0165 potential issue"
                    !WRITE(*,*) "NEMS data"
                    !write(*,*) waterlevel_ptr(test_loop)
                    !write(*,*) "EC Module data"
                    !write(*,*) zbndz(test_loop)
                    ...
                    !write(*,*) xbndz(test_loop)
                    !write(*,*) ybndz(test_loop)
                    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0790") then
                    !write(*,*) "CONT_BND_0790 potential issue"
                    !WRITE(*,*) "NEMS data"
                    !write(*,*) waterlevel_ptr(test_loop)
                    !write(*,*) "EC Module data"
                    !write(*,*) zbndz(test_loop)
        ...
        elseif(itemNameList(iIndex)=='surface_eastward_sea_water_velocity_dflow') then
                ...
                if (ESMF_STDERRORCHECK(rc)) return
                !WRITE(*,*) "Vlex ptr data"
                !WRITE(*,*) DFLOW_VBC_ID(1:5)
                !WRITE(*,*) velx_ptr(1:5)
                    if(nbndt > 0) then
                    DO test_loop2= 1, nbndz
                    if(idbndz(test_loop2)(1:13) .eq. "CONT_BND_0100") then
                    DO test_loop = 1, size(DFLOW_VBC_lat)
                    !if(waterlevel_ptr(test_loop)*0.0 .ne. 0.0) then
                    !WRITE(*,*) "Water level data has NAN Value"
                    if(lat(test_loop) .eq. ybndt(test_loop2)) then
                    write(*,*) "velocity issue"
                    write(*,*) idbndz(test_loop2)
                    write(*,*) DFLOW_VBC_ID(test_loop)
                    write(*,*) velx_ptr(test_loop)
                    !write(*,*) velx_ptr(test_loop)
                    !write(*,*) "coordinates"
                    !write(*,*) xbndt(test_loop)
                    !write(*,*) ybndt(test_loop)
                    ENDIF
                    ENDDO
                    ENDIF
                    ENDDO
    ...
    if(nbndn > 0 .and. vel_connection == .TRUE.) then
       ...
       allocate(TanV(nbndn))
       allocate(NorV(nbndn))
       allocate(TanV_left(nbndn))
       allocate(NorV_left(nbndn))
       allocate(TanV_right(nbndn))
       allocate(NorV_right(nbndn))
       ...
       vel : do vel_loop = 1, nbndn
          dir : do dir_loop = 1, size(DFLOW_VBC_UTANx)
             if(ybndt(vel_loop) .eq. DFLOW_VBC_lat(dir_loop)) then
                ...
                TanV_left(vel_loop) = (DFLOW_VBC_UTANx_left(dir_loop) * velx_ptr(dir_loop*2-1)) + (DFLOW_VBC_UTANy_left(dir_loop) * vely_ptr(dir_loop*2-1))
                NorV_left(vel_loop) = (DFLOW_VBC_UNORx_left(dir_loop) * velx_ptr(dir_loop*2-1)) + (DFLOW_VBC_UNORy_left(dir_loop) * vely_ptr(dir_loop*2-1))
                TanV_right(vel_loop) = (DFLOW_VBC_UTANx_right(dir_loop) * velx_ptr(dir_loop*2)) + (DFLOW_VBC_UTANy_right(dir_loop) * vely_ptr(dir_loop*2))
                NorV_right(vel_loop) = (DFLOW_VBC_UNORx_right(dir_loop) * velx_ptr(dir_loop*2)) + (DFLOW_VBC_UNORy_right(dir_loop) * vely_ptr(dir_loop*2))
                exit dir
             endif
          enddo dir
        enddo vel
       ...
       !if(time_user .eq. tstart_user) then
       !    zbndt_t0(1:nbndt) = 0.0
       !    zbndn_t0(1:nbndn) = 0.0
       !    zbndt_t1(1:nbndt) = TanV(1:nbndt)
       !    zbndn_t1(1:nbndn) = NorV(1:nbndn)
       !else
       !    zbndt_t0(1:nbndt) = zbndt_t1(1:nbndt)
       !    zbndn_t0(1:nbndn) = zbndn_t1(1:nbndn)
       !    zbndt_t1(1:nbndt) = TanV(1:nbndt)
       !    zbndn_t1(1:nbndn) = NorV(1:nbndn)                
       !endif
       ...
       if(time_user .eq. tstart_user) then
           ...
           vel_index = 1 
           do i=1, nbndt            
           zbndt_t1(vel_index) = TanV_left(i)
           zbndn_t1(vel_index) = NorV_left(i)
           zbndt_t1(vel_index+1) = TanV_right(i)
           zbndn_t1(vel_index+1) = NorV_right(i)
           vel_index = vel_index + 1
           end do
       else
           do i=1, nbndt
           zbndt_t0(vel_index) = zbndt_t1(vel_index)
           zbndn_t0(vel_index) = zbndn_t1(vel_index)
           zbndt_t0(vel_index+1) = zbndt_t1(vel_index+1)
           zbndn_t0(vel_index+1) = zbndn_t1(vel_index+1)
           zbndt_t1(vel_index) = TanV_left(i)
           zbndn_t1(vel_index) = NorV_left(i)
           zbndt_t1(vel_index+1) = TanV_right(i)
           zbndn_t1(vel_index+1) = NorV_right(i)
       ...
       deallocate(TanV)
       deallocate(NorV)
       deallocate(TanV_left)
       deallocate(NorV_left)
       deallocate(TanV_right)
       deallocate(NorV_right)
    endif
   ...
   !    WRITE(*,*) zbndq(1:nbndu)
    ...
    write(*,*) "DFlow init times before loop"
    write(*,*) t0
    write(*,*) t1
    write(*,*) "dt user"
    write(*,*) dt_user
    ...
    do while (time1 .lt. tstop_user .and. iresult==DFM_NOERR)  ! time loop
   
        !time_user = time_user + dts

        timetarget = time1 + epsilon(1d0)
        timetarget = min(timetarget, tstop_user)

        a1 = (timetarget - t0)/(t1-t0)
        a0 = 1.0 - a1
        
        !dt_user_count = dt_user_count + dts

        !if(dt_user_count >= dt_user) then
        ...
        !write(*,*) "Linear time interp coefficents"
        !write(*,*) a1
        !write(*,*) a0
        ...
        !if(nbndz > 0) then
        !    zbndz(1:nbndz) = (zbndz_t0(1:nbndz) * (a1+a0)) + ((zbndz_t1(1:nbndz) - zbndz_t0(1:nbndz))*a1)
        !endif

        !if(nbndu > 0) then
        !    zbndq(1:nbndu) = (zbndq_t0(1:nbndu) * (a1+a0)) + ((zbndq_t1(1:nbndu) - zbndq_t0(1:nbndu))*a1)
        !endif

        if(nbndu > 0) then
            DFLOW_zbndq_temp_interp(:) = (DFLOW_zbndq_t0(:) * (a1+a0)) + ((DFLOW_zbndq_t1(:) - DFLOW_zbndq_t0(:))*a1)

            test_loop = 1
            do test_loop2 = 1, nbndu
                zbndq(test_loop2) = (DFLOW_zbndq_temp_interp(test_loop)*qid_left_weight(test_loop2)) + (DFLOW_zbndq_temp_interp(test_loop+1)*qid_right_weight(test_loop2))
                test_loop = test_loop + 2
            enddo
         endif

        if(nbndz > 0) then
            DFLOW_zbndz_temp_interp(:) = (DFLOW_zbndz_t0(:) * (a1+a0)) + ((DFLOW_zbndz_t1(:) - DFLOW_zbndz_t0(:))*a1)

            test_loop = 1
            do test_loop2 = 1, nbndz
                zbndz(test_loop2) = (DFLOW_zbndz_temp_interp(test_loop)*wl_left_weight(test_loop2)) + (DFLOW_zbndz_temp_interp(test_loop+1)*wl_right_weight(test_loop2))
                test_loop = test_loop + 2
            enddo  
        ...
        if(numlatsg > 0) then
            qplat(1:numlatsg) = (qplat_t0(1:numlatsg) * (a1+a0)) + ((qplat_t1(1:numlatsg) - qplat_t0(1:numlatsg))*a1)
        endif

        if(nbndn > 0 .and. vel_connection == .TRUE.) then

            DFLOW_zbndt_temp_interp(:) = (DFLOW_zbndt_t0(:) * (a1+a0)) + ((DFLOW_zbndt_t1(:) - DFLOW_zbndt_t0(:))*a1)
            DFLOW_zbndn_temp_interp(:) = (DFLOW_zbndn_t0(:) * (a1+a0)) + ((DFLOW_zbndn_t1(:) - DFLOW_zbndn_t0(:))*a1)

            test_loop = 1
            do test_loop2 = 1, nbndt
                zbndt(test_loop2) = (DFLOW_zbndt_temp_interp(test_loop)*wl_left_weight(test_loop2)) + (DFLOW_zbndt_temp_interp(test_loop+1)*wl_right_weight(test_loop2))
                zbndn(test_loop2) = (DFLOW_zbndn_temp_interp(test_loop)*wl_left_weight(test_loop2)) + (DFLOW_zbndn_temp_interp(test_loop+1)*wl_right_weight(test_loop2))
                test_loop = test_loop + 2
            enddo
        ...
        !call flow_run_sometimesteps(-1,iresult)
        !call flow_run_single_timestep(dummykey, iresult)
        ...
        !time0 = time0 + dts
        !if(nbndz > 0) then
        !write(*,*) "waterlevel EC-Module vs. NEMS difference"
        !write(*,*) maxval(zbndz_test(1:nbndz)-zbndz(1:nbndz))
        !write(*,*) minval(zbndz_test(1:nbndz)-zbndz(1:nbndz))

        !write(*,*) "waterlevel NEMS and EC Module"
        !write(*,*) zbndz_test(1:2)
        !write(*,*) zbndz(1:2)
        !do test_loop = 1, nbndz
        !diff = (zbndz_test(test_loop) - zbndz(test_loop))/zbndz(test_loop)*100
        !if(abs(diff) > 5.0) then
        !write(*,*) "NEMS and EC Module water levels"
        !write(*,*) zbndz_test(test_loop)
        !write(*,*) zbndz(test_loop)
        !write(*,*) idbndz(test_loop)
        !endif
        !enddo
        !endif
       
        !if(nbndu > 0) then
        !write(*,*) "discharge EC-Module vs. NEMS difference"
        !write(*,*) maxval(zbndq_test(1:nbndu)-zbndq(1:nbndu))
        !write(*,*) minval(zbndq_test(1:nbndu)-zbndq(1:nbndu))

        !write(*,*) "discharge NEMS and EC Module"
        !write(*,*) zbndq_test(1:2)
        !write(*,*) zbndq(1:2)
        !do test_loop = 1, nbndu
        !diff = (zbndq_test(test_loop) - zbndq(test_loop))/zbndq(test_loop)*100
        !if(abs(diff) > 5.0) then
        !write(*,*) "NEMS and EC Module discharge"
        !write(*,*) zbndq_test(test_loop)
        !write(*,*) zbndq(test_loop)
        !write(*,*) idbndu(test_loop)
        !endif
        !enddo
        !endif

       !if(numlatsg > 0) then
       ! write(*,*) "lateral discharge EC-Module vs. NEMS difference"
       ! write(*,*) maxval(qplat_test(1:numlatsg)-qplat(1:numlatsg))
       ! write(*,*) minval(qplat_test(1:numlatsg)-qplat(1:numlatsg))

       ! write(*,*) "lateral discharge NEMS and EC Module"
       ! write(*,*) qplat_test(1:4)
       ! write(*,*) qplat(1:4)
       ! do test_loop = 1, numlatsg
       ! diff = (qplat_test(test_loop) - qplat(test_loop))/qplat(test_loop)*100
       ! if(abs(diff) > 5.0) then
       ! write(*,*) "NEMS and EC Module lateral discharge"
       ! write(*,*) qplat_test(test_loop)
       ! write(*,*) qplat(test_loop)
       ! write(*,*) lat_ids(test_loop)
       ! endif
       ! enddo
       !endif
    ...
    write(*,*) "DFLOWFM has reached end of hourly time loop"
    write(*,*) "Linear time interp coefficents"
    write(*,*) a1
    write(*,*) a0
    write(*,*) "Time variables"
    ...
    !write(*,*) "Waterlevels min and max after NEMS"
    !write(*,*) maxval(s1)
    !write(*,*) minval(s1)
    ...
    !if(nbndu > 0) then
    !do x = 1, nbndu
    !write(DFLOW_Q_FILE,*) zbndq(x)
    !enddo
    !endif

    !if(numlatsg > 0) then
    !do x = 1, numlatsg
    !write(DFLOW_LATQ_FILE,*) qplat(x)
    !enddo
    !endif
    ...
    !DO test_loop = 1, nbndz
    !if(idbndz(test_loop)(1:13) .eq. "CONT_BND_0100") then
    !     write(*,*) "CONT_BND_0100 potential issue after model execution"
    !     write(*,*) "EC Module data"
    !     write(*,*) zbndz(test_loop)
    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0010") then
    !     write(*,*) "CONT_BND_0010 potential issue after model execution"
    !     write(*,*) "EC Module data"
    !     write(*,*) zbndz(test_loop)
    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0020") then
    !     write(*,*) "CONT_BND_0020 potential issue after model execution"
    !     write(*,*) "EC Module data"
    !     write(*,*) zbndz(test_loop)
    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0040") then
    !     write(*,*) "CONT_BND_0040 potential issue after model execution"
    !     write(*,*) "EC Module data"
    !     write(*,*) zbndz(test_loop)
    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0045") then
    !     write(*,*) "CONT_BND_0045 potential issue after model execution"
    !     write(*,*) "EC Module data"
    !     write(*,*) zbndz(test_loop)
    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0790") then
    !     write(*,*) "CONT_BND_0790 potential issue after model execution"
    !     write(*,*) "EC Module data"
    !     write(*,*) zbndz(test_loop)
    !elseif(idbndz(test_loop)(1:13) .eq. "CONT_BND_0075") then
    !     write(*,*) "CONT_BND_0075 potential issue after model execution"
    !     write(*,*) "EC Module data"
    !     write(*,*) zbndz(test_loop)
