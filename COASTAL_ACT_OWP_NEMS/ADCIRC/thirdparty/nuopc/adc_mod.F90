...
module adc_mod
  ...
  use MESH   , only: np,ne,nm,slam,sfea,x,y,ics
...
#define METHOD "locstream_waterlevel"
    ...
    function locstream_waterlevel(localPet, rc, the_data)
        ! Return value
        ...
        real(ESMF_KIND_R8), allocatable    :: lat(:),lon(:), agg_lat(:), agg_lon(:), lat_total(:), lon_total(:), x_test(:), y_test(:), z_test(:)
        integer(ESMF_KIND_I4), allocatable :: mask(:), id(:)
        real(ESMF_KIND_R8) :: theta, phi
        integer :: node_total, pet_loop, thread_count, mesh_count

        rc = ESMF_SUCCESS

        !-------------------------------------------------------------------
        ! Initalize and calculate decimal degrees coordinates of ADCIRC mesh
        ! Flag mask to indicate that there is data everywhere for initalized
        ! array

        ! details Get MPI_communicator from ESMF VM.
        !call ESMF_VMGet(vm, mpiCommunicator=adcirc_comm, rc=rc)
        !if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        !line=__LINE__, &
        !file=__FILE__)) &
        !return  ! bail out
        ...
        !!$OMP PARALLEL
        !!$OMP BARRIER
        !call MPI_ALLREDUCE(the_data%NumOwnedNd,node_total,1,MPI_INTEGER,MPI_MAX,adcirc_comm,ierr) 
        !allocate(agg_lat(node_total))
        !allocate(agg_lon(node_total))
       
        !agg_lat(:) = 0.0
        !agg_lon(:) = 0.0

        !do pet_loop = 1, size(sfea)
        !agg_lat(pet_loop) = sfea(pet_loop)*180/3.14159265359
        !agg_lon(pet_loop) = slam(pet_loop)*180/3.14159265359
        !enddo

        !call MPI_Comm_size(adcirc_comm, thread_count, ierr)

        !ALLOCATE(lat_total(node_total*thread_count))
        !write(*,*) "mesh coordinate system"
        !write(*,*) ics
        
   
        allocate(lat(size(sfea)))
        allocate(lon(size(sfea)))
        allocate(mask(size(sfea)))
        numlocations = size(sfea)
        do i=1, numlocations, 1
            lat(i) = sfea(i)*180/3.14159265359
            lon(i) = slam(i)*180/3.14159265359
            mask(i) = 0
        enddo
        ...
        call ESMF_LogWrite("Initalizing ADCIRC waterlevel locstream", ESMF_LOGMSG_INFO)
        !-------------------------------------------------------------------
...
#define METHOD "locstream_velx"
    ...
    function locstream_velx(localPet, rc, the_data)
        ! Return value
        ...
        call ESMF_LogWrite("Initalizing ADCIRC velx locstream", ESMF_LOGMSG_INFO)
        !-------------------------------------------------------------------
        ! Create the LocStream:  Allocate space for the LocStream object,
        ! define the number and distribution of the locations.
        !-------------------------------------------------------------------
...
#define METHOD "locstream_vely"
    ...
    function locstream_vely(localPet, rc, the_data)
        ! Return value
        ...
        call ESMF_LogWrite("Initalizing ADCIRC vely locstream", ESMF_LOGMSG_INFO)
        !-------------------------------------------------------------------
        ! Create the LocStream:  Allocate space for the LocStream object,
        ! define the number and distribution of the locations.
        !-------------------------------------------------------------------
