...
! $HeadURL: https://svn.oss.deltares.nl/repos/delft3d/branches/releases/140712/src/engines_gpl/dflowfm/packages/dflowfm_kernel/src/dflowfm_kernel/prepost/flow_initexternalforcings.f90 $
 ...
 if (allocated(kbndz))        deallocate(xbndz,ybndz,idbndz,xy2bndz,zbndz,kbndz,zbndz0)
 ...
 if (nbndz > 0) then                                 ! now you know the elementsets for the waterlevel bnds
    allocate ( xbndz(nbndz), ybndz(nbndz), idbndz(nbndz), xy2bndz(2,nbndz), zbndz(nbndz), kbndz(n4,nbndz), zbndz0(nbndz), kdz(nbndz) , stat=ierr     )
    call aerr('xbndz(nbndz), ybndz(nbndz), idbndz(nbndz), xy2bndz(2,nbndz), zbndz(nbndz), kbndz(n4,nbndz), zbndz0(nbndz), kdz(nbndz)', ierr, nbndz*10 )
 ...
 if (nbndu > 0) then                                 ! similar for u bnd's
    allocate ( xbndu(nbndu), ybndu(nbndu), idbndu(nbndu), xy2bndu(2,nbndu), kbndu(n4,nbndu), kdu(nbndu) , stat=ierr)
    call aerr('xbndu(nbndu), ybndu(nbndu), idbndu(nbndu), xy2bndu(2,nbndu), kbndu(n4,nbndu), kdu(nbndu)', ierr, nbndu*(n4+5) )
